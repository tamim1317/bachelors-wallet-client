...
import { MessageCircle, Send, Trash2, Megaphone, Users, Wifi } from 'lucide-react';
...
export default function ChatPage() {
  ...
  const handleTyping = () => {
    socketRef.current?.emit('chat:typing', { userId: user._id, name: user.name });
  };
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      socketRef.current?.emit('chat:message', res.data.data);
      setInput('');
      setType('text');
    } catch (err) {
  ...
  style={{ height: 'calc(100dvh - 130px)' }}>
    ...
    style={{ overscrollBehavior: 'contain' }}>
        {messages.length === 0 && (
          <div className="flex items-center justify-center h-full">
            <div className="text-center text-gray-400 dark:text-gray-600">
              ...
              <p className="text-sm">এখনো কোনো message নেই</p>
        ...
        {messages.map((msg, i) => {
          ...
          return (
            <div key={msg._id || i} className={`flex gap-2 group ${isOwn ? 'flex-row-reverse' : ''}`}>
              {/* Avatar */}
              {!isOwn && (
                ...
                {!isOwn && (
                  <div className="flex items-center gap-1.5">
                    ...
                    {msg.sender?.role === 'manager' && (
                      <span className="text-xs bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 px-1.5 py-0.5 rounded-md">
                        👑
                      </span>
                    )}
                  </div>
                ...
                <div className={`relative px-4 py-2.5 rounded-2xl text-sm ${
                  ...
                  {(isOwn || isManager) && (
                    <button
                      ...
                      className={`absolute -top-2 ${isOwn ? '-left-2' : '-right-2'} w-5 h-5 rounded-full bg-red-500 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all`}>
                      <Trash2 size={10} />
                    </button>
                  )}
                </div>
        ...
        {typing && (
          <div className="flex gap-2">
            <div className="px-4 py-2.5 bg-gray-100 dark:bg-gray-800 rounded-2xl rounded-tl-sm">
